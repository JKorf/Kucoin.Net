...
[![Docs](https://img.shields.io/badge/Docs-Kucoin.Net-1b7f50?style=for-the-badge)](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Kucoin.Net)
...
The [Kucoin.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Kucoin.Net) is the main resource for installing, configuring, and using the library.

| Resource | Description |
|--|--|
| [Client guide](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Kucoin.Net) | Installation, REST and WebSocket clients, authentication, dependency injection, error handling, and advanced features |
| [Examples](https://cryptoexchange.jkorf.dev/docs/exchange-clients/examples?library=Kucoin.Net) | Common REST and WebSocket operations |
| [API reference](https://cryptoexchange.jkorf.dev/docs/exchange-clients/reference?library=Kucoin.Net) | Client interfaces, methods, and properties |
...
For more examples and explanations, continue with the [Kucoin.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Kucoin.Net) or browse the [compilable repository examples](https://github.com/JKorf/Kucoin.Net/tree/master/Examples).
...
V2 uses a strict interface per capability. The `SharedApi` property on each Kucoin API surface therefore exposes only the operations or subscriptions that surface actually supports. `IKucoinSharedApiClient` groups the `SpotRest`, `FuturesRest`, `SpotSocket`, and `FuturesSocket` surfaces for dependency injection and runtime capability lookup.
...
using var restClient = new KucoinRestClient();
...
The request and response models belong to `CryptoExchange.Net.SharedApis`, so the same operation can accept another exchange's `IGetTickerRest` implementation. When using dependency injection, inject `IKucoinSharedApiClient` to access all Kucoin Shared API surfaces or inject a capability such as `IGetTickerRest` directly. Use `GetCapability` on the aggregate when the operation, transport, or trading mode is selected at runtime.
...
* Version 9.6.0 - 24 Sep 2026
    ...
    * Shared APIs
      ...
      * Added `(I)KucoinSharedApiClient` containing all V2 SharedApi references
      * Added `SharedApi` to `KucoinOptions` to provide SharedApi related options
    ...
    * Fixed proxy setting not being used for internal REST requests
...
* Version 9.2.0 - 21 Jul 2026
    ...
    * Updated GetTickersAsync request weight from 15 to 5
