...
For code that needs to work across multiple exchanges, use `CryptoExchange.Net.SharedApis` interfaces accessed via `.SharedApi` properties. Kucoin exposes shared clients on `SpotApi` and `FuturesApi`.
