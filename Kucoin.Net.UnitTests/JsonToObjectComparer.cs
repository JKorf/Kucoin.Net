    ...
    public class JsonToObjectComparer<T>
    {
           ...
           List<string> takeFirstItemForCompare = null)
        {
            ...
            foreach (var method in callResultMethods)
            {
                ...
                if (useNestedObjectPropertyForCompare?.ContainsKey(method.Name) == true)
                    resultData = resultData.GetType().GetProperty(useNestedObjectPropertyForCompare[method.Name]).GetValue(resultData);
